...
use perlin::PerlinNoiseHarmonic;     // perlin noise generator
...
fn generate_perlin_picture() -> Result<(), std::io::Error> {
    ...
    // create a new harmonic perlin noise
    ...
    let harmonic_perlin_noise = PerlinNoiseHarmonic::new(picture_width, 6, true, 4).unwrap();
    ...
    for j in 0..picture_width {
        for i in 0..picture_width {
            let gray: u8 = harmonic_perlin_noise.get_pixel_value(i, j);
